...
export default function LoginScreen() {
  ...
  const [forcaSenha, setForcaSenha] = useState(0); // Lógica da barrinha
  ...
  // Lógica para medir a força da senha
  useEffect(() => {
    let nivel = 0;
  ...
  const handleEsqueciSenha = async () => {
    if (!email) return Alert.alert("Atenção", "Digite seu e-mail para recuperar a senha.");
    try {
  ...
  const handleAuth = async () => {
    ...
    } catch (e) { Alert.alert("Falha", "Verifique suas credenciais."); }
  ...
  return (
    <View style={styles.mainContainer}>
      <ImageBackground 
        source={require('../assets/images/TelaLogin.png')} 
          ...
          <ScrollView contentContainerStyle={styles.scrollContent} bounces={false}>
            <View style={styles.overlay}>
              ...
              <View style={styles.card}>
                ...
                <TextInput 
                  style={styles.input} 
                  placeholder="agente@detetive.com" 
                ...
                {/* BARRINHA DE SEGURANÇA (Aparece apenas no cadastro) */}
                ...
                <TouchableOpacity style={styles.secondaryButton} onPress={() => setIsLogin(!isLogin)}>
                  <Text style={styles.secondaryButtonText}>
                    {isLogin ? 'Não tem conta? Registre-se' : 'Já é um agente? Login'}
                  </Text>
                </TouchableOpacity>
              </View>
